...
/**
 * Get the position of the square shifted inward by 0.5 square size
 */
...
export class Cube extends Group {
    ...
    /**
     * Rotate one face
     * @param mousePrePos Mouse screen coordinates before rotation
     * @param mouseCurPos Current mouse screen coordinates
     * @param controlSquare Controlled square
     * @param camera Camera
    ...
    public rotateOnePlane(mousePrePos: Vector2, mouseCurPos: Vector2, controlSquare: SquareMesh, camera: Camera, winSize: {w: number; h: number}) {
        ...
        if (!this.state.inRotation) {
            ...
            // Other squares on the same face as controlSquare
            ...
            // square1 and square2 are two SquareMeshes in the vertical and horizontal directions on the same face
            ...
            // Record the four possible rotation directions
            ...
            // Determine rotation direction based on the angle between the four possible direction vectors and mouse movement direction; the direction with the smallest angle is the rotation direction
            let minAngle = Math.abs(getAngleBetweenTwoVector2(squareDirs[0].screenDir, screenDir));
            let rotateDir = squareDirs[0];  // Final determined rotation direction
            ...
            // Rotation axis: calculated using cross product of normal vector and rotation direction
            const rotateDirLocal = rotateDir.endSquare.element.pos.clone().sub(rotateDir.startSquare.element.pos).normalize();
            const rotateAxisLocal = squareNormal.clone().cross(rotateDirLocal).normalize(); // Rotation axis

            // Rotating squares: the vector from controlSquare position to the position of the square to be rotated is perpendicular to the rotation axis; this characteristic can be used to filter out all squares to be rotated
        ...
        // Rotation angle: use the projection length of screenDir in the rotation direction; the longer the projection length, the larger the rotation angle
        // The positive/negative value of the projection length affects the direction of the cube rotation angle
        // Rotation angle = projection length / cube size * 90 degrees
    ...
    public getAfterRotateAnimation() {
        ...
        const rotateSpeed = Math.PI * 0.5 / 500; // 1 second to rotate 90 degrees
        let rotatedAngle = 0;
        let lastTick: number;
    ...
    /**
     * Update state after rotation
     */
    private updateStateAfterRotate() {
        // Rotate to correct position; sometimes the rotation is not a multiple of 90 degrees, need to correct to a multiple of 90 degrees
        const needRotateAnglePI = this.getNeededRotateAngle();
        this.state.rotateAnglePI += needRotateAnglePI;

        // Update data: CubeElement state, normal vectors, positions, etc. have changed after rotation
        const angleRelative360PI = this.state.rotateAnglePI % (Math.PI * 2);
        // const timesOfRight = angleRelative360PI / rightAnglePI; // How many 90 degrees the rotation angle is equivalent to

        if (Math.abs(angleRelative360PI) > 0.1) {

            // Update positions and normal vectors
            ...
            for (let i = 0; i < this.state.activeSquares.length; i++) {
                ...
                nor.applyMatrix4(rotateMat2); // Normal vector after rotation
                pos.applyMatrix4(rotateMat2); // Position after rotation

                // Find the square corresponding to after rotation and update its color
    ...
    /**
     * Get standard screen coordinates of Square
     */
